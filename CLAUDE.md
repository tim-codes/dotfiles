...
- Markdown reading: the `md` function (fish `src/.config/fish/common.fish`, bash `src/.bashrc`) renders a file or stdin — mdcat at full terminal width when the doc contains tables (glow pads every table column to the same width), glow at a 100-col prose measure otherwise. Deliberately not piped through bat: both renderers drop all colour as soon as stdout is a pipe, so mdcat is forced with `--ansi` before `less -R` and glow renders straight to the terminal, using its own `-p` pager only when the output overflows the screen. glow's config is stowed at `src/.config/glow/glow.yml`; on macOS `scripts/setup-macbook` symlinks `~/Library/Preferences/glow/glow.yml` to it because glow reads only that path and ignores `--config`/`GLOW_CONFIG`.
