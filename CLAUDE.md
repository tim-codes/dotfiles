...
- Markdown reading: the `md` function (fish `src/.config/fish/common.fish`, bash `src/.bashrc`) renders a file or stdin through bat — mdcat at full terminal width when the doc contains tables (glow pads every table column to the same width), glow at a 100-col prose measure otherwise. glow's config is stowed at `src/.config/glow/glow.yml`; on macOS `scripts/setup-macbook` symlinks `~/Library/Preferences/glow/glow.yml` to it because glow reads only that path and ignores `--config`/`GLOW_CONFIG`.
